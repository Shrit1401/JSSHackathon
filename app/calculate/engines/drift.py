...
from ..models import (
    ...
    DriftSeverity, FeatureDrift, DriftResult, DeviceDriftState, DriftEvent,
)
from .baseline import BaselineEngine, NUMERIC_FEATURES
...
class DriftDetector:
    ...
    def _compute_feature_drifts(self, fv: DeviceFeatureVector, baseline) -> list[FeatureDrift]:
        ...
        for feat in NUMERIC_FEATURES:
            ...
            drifts.append(FeatureDrift(
                feature_name=feat, z_score=round(z, 4), abs_z_score=round(abs_z, 4),
                baseline_mean=stats.mean, baseline_std=stats.std,
                current_value=round(current, 4), is_drifting=is_feat_drifting,
    ...
    def _emit_event(self, device_id, device_type, event_type, severity, drift_score, window_id, drifting_features, description):
    ...
    def analyze(self, feature_vector: DeviceFeatureVector) -> DriftResult:
        ...
        if not state:
            state = DeviceDriftState(
                ...
                current_severity=DriftSeverity.NONE, is_drifting=False,
                consecutive_drift_windows=0, max_consecutive_drift=0,
                total_drift_windows=0, total_windows_analyzed=0,
                latest_drift_score=0.0, peak_drift_score=0.0,
                currently_drifting_features=[], historically_drifted_features=[],
            )
        ...
        if is_drifting:
            ...
            if prev_was_drifting:
                self._emit_event(device_id, device_type, "drift_resolved", DriftSeverity.NONE, 0.0, window_id, [], f"{device_id} behavior returned to baseline")
        ...
        if is_drifting and state.consecutive_drift_windows == 1:
            self._emit_event(device_id, device_type, "drift_detected", severity, drift_score, window_id, drifting_names, f"{device_id} showing anomalous behavior in {len(drifting_names)} features")
        if is_drifting and state.consecutive_drift_windows == CONFIRMATION_WINDOWS:
            self._emit_event(device_id, device_type, "drift_confirmed", severity, drift_score, window_id, drifting_names, f"{device_id} drift confirmed after {CONFIRMATION_WINDOWS} consecutive windows — top deviation: {drifting_features[0].feature_name} (z={drifting_features[0].z_score:+.2f})")
        if is_drifting and severity != prev_severity and state.consecutive_drift_windows > 1:
            self._emit_event(device_id, device_type, "severity_escalated", severity, drift_score, window_id, drifting_names, f"{device_id} drift severity escalated from {prev_severity.value} to {severity.value}")
