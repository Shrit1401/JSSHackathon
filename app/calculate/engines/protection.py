...
from ..models import (
    DeviceType, DeviceFeatureVector,
    GateDecision, ProtectionStatus, GateEvent, PoisoningAttempt,
    DeviceProtectionState, ProtectionSummary,
)
...
class BaselineProtector:
    ...
    def gate_check(self, device_id: str, device_type: DeviceType, trust_score: float, feature_vector: Optional[DeviceFeatureVector] = None) -> GateDecision:
        ...
        if has_drift:
            self._record_poisoning(
                device_id, device_type, trust_score, ProtectionStatus.FROZEN, feature_vector,
        ...
        if self._consecutive_denied[device_id] >= QUARANTINE_AFTER_DENIALS:
            ...
            decision = GateDecision.QUARANTINED
            reason = f"Trust {trust_score:.1f} < {TRUST_GATE_THRESHOLD} for {self._consecutive_denied[device_id]} consecutive cycles — ESCALATED TO QUARANTINE"
        else:
            decision = GateDecision.DENIED
            reason = f"Trust {trust_score:.1f} < {TRUST_GATE_THRESHOLD} — baseline frozen (denial streak: {self._consecutive_denied[device_id]}/{QUARANTINE_AFTER_DENIALS})"

        self._log_gate_event(device_id, device_type, decision, trust_score, reason, now)
        return decision

    def _log_gate_event(self, device_id, device_type, decision, trust_score, reason, timestamp):
        self.gate_events.append(GateEvent(
            device_id=device_id, device_type=device_type, decision=decision,
            trust_score=trust_score, threshold=TRUST_GATE_THRESHOLD,
            reason=reason, timestamp=timestamp,
        ))

    def _record_poisoning(self, device_id, device_type, trust_score, status, feature_vector, reason):
        has_drift = False
        if feature_vector:
            try:
                dev = self.baseline_engine.compute_deviation(feature_vector)
    ...
    def get_device_state(self, device_id: str) -> Optional[DeviceProtectionState]:
        ...
        return DeviceProtectionState(
            device_id=device_id, device_type=device_type, status=status,
            is_frozen=is_frozen, is_quarantined=is_quarantined, trust_score=trust,
            consecutive_denied=self._consecutive_denied.get(device_id, 0),
            total_allowed=allowed, total_denied=denied,
    ...
    def get_summary(self) -> ProtectionSummary:
        ...
        return ProtectionSummary(
            total_devices=len(states), devices_learning=learning,
            devices_frozen=frozen, devices_quarantined=quarantined,
