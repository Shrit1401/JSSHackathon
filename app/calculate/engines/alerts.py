...
from ..models import (
    DeviceType, Confidence, DriftSeverity,
    AlertType, AlertSeverity, Alert, AlertSummary,
)
...
class AlertManager:
    def __init__(self, drift_detector: DriftDetector, policy_engine: PolicyEngine, ml_detector: MLDetector, trust_engine: TrustEngine):
    ...
    def scan_policy_violations(self):
        for violation in self.policy_engine.violations:
            self._add(Alert(
                alert_id=str(uuid.uuid4()), alert_type=AlertType.POLICY_VIOLATION,
                severity=CONFIDENCE_TO_SEVERITY.get(violation.confidence, AlertSeverity.MEDIUM),
                device_id=violation.device_id, device_type=violation.device_type,
                title=f"Policy violation: {violation.rule_id}", reason=violation.description,
                evidence={"rule_id": violation.rule_id, "policy_type": violation.policy_type.value, "evidence": violation.evidence},
    ...
    def scan_ml_anomalies(self):
        for score in self.ml_detector.get_anomalous_devices():
            ...
            self._add(Alert(
                alert_id=str(uuid.uuid4()), alert_type=AlertType.ML_ANOMALY,
                severity=severity, device_id=score.device_id, device_type=score.device_type,
                ...
                evidence={"anomaly_score": score.anomaly_score, "raw_score": score.raw_score, "threshold": score.threshold, "feature_contributions": score.feature_contributions},
        ...
        for event in self.drift_detector.events:
            if event.event_type in ("drift_confirmed", "severity_escalated"):
                self._add(Alert(
                    ...
                    device_id=event.device_id, device_type=event.device_type,
                    title=f"Behavioral drift {event.event_type.replace('_', ' ')}",
                    reason=event.description,
                    evidence={"drift_score": event.drift_score, "drifting_features": event.drifting_features, "window_id": event.window_id},
    ...
    def scan_trust_drops(self):
        for score in self.trust_engine.get_all_latest():
            if score.trust_score < 40:
                ...
                continue
            self._add(Alert(
                alert_id=str(uuid.uuid4()), alert_type=AlertType.TRUST_DROP,
                severity=severity, device_id=score.device_id, device_type=score.device_type,
                ...
                evidence={"trust_score": score.trust_score, "risk_level": score.risk_level.value, "breakdown": score.signal_breakdown.model_dump()},
                trust_score_at_time=score.trust_score, timestamp=score.timestamp,
            ))

    def scan_quarantine_events(self):
        for state in self.trust_engine.protector.get_quarantined_devices():
            self._add(Alert(
                alert_id=str(uuid.uuid4()), alert_type=AlertType.DEVICE_QUARANTINED,
                severity=AlertSeverity.CRITICAL, device_id=state.device_id, device_type=state.device_type,
    ...
    def scan_poisoning_attempts(self):
        for attempt in self.trust_engine.protector.poisoning_attempts:
            self._add(Alert(
                alert_id=str(uuid.uuid4()), alert_type=AlertType.POISONING_ATTEMPT,
                severity=AlertSeverity.HIGH, device_id=attempt.device_id, device_type=attempt.device_type,
                title=f"Baseline poisoning attempt: {attempt.device_id}",
                reason=attempt.reason,
                evidence={"trust_score": attempt.trust_score_at_time, "drift_detected": attempt.feature_drift_detected, "protection_status": attempt.protection_status.value},
                trust_score_at_time=attempt.trust_score_at_time, timestamp=attempt.timestamp,
            ))
    ...
    def get_alerts(self, alert_type: Optional[AlertType] = None, severity: Optional[AlertSeverity] = None, device_id: Optional[str] = None) -> list[Alert]:
        result = self.alerts
        if alert_type:
    ...
    def get_summary(self) -> AlertSummary:
        ...
        return AlertSummary(
            total_alerts=len(self.alerts), by_type=dict(by_type),
            by_severity=dict(by_severity), by_device=dict(by_device),
            unacknowledged=unack, most_recent=max((a.timestamp for a in self.alerts), default=None),
        )
