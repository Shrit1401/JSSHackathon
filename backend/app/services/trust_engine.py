...
from app.services.baseline_protector import BaselineProtector
...
class TrustEngine:
    ...
    def compute_trust(self, device_id: str, device_type: DeviceType) -> DeviceTrustScore:
        ...
        breakdown = SignalBreakdown(
            ...
            policy_high_confidence=high_conf_count,
    ...
    def get_all_latest(self) -> list[DeviceTrustScore]:
        results = []
        for device_id, scores in self.history.items():
            if scores:
                results.append(scores[-1])
        return sorted(results, key=lambda s: s.trust_score)

    def get_devices_by_risk(self, risk_level: RiskLevel) -> list[DeviceTrustScore]:
        return [s for s in self.get_all_latest() if s.risk_level == risk_level]
    ...
    def get_device_history(self, device_id: str) -> Optional[DeviceTrustHistory]:
        ...
        return DeviceTrustHistory(
            device_id=device_id,
            device_type=scores[0].device_type,
            scores=scores,
            current_score=trust_vals[-1],
            current_risk=scores[-1].risk_level,
            lowest_score=min(trust_vals),
            highest_score=max(trust_vals),
    ...
    def get_summary(self) -> TrustSummary:
        ...
        return TrustSummary(
            ...
            devices_by_risk=dict(by_risk),
            average_trust=avg,
            lowest_trust_device=lowest_dev,
            lowest_trust_score=lowest_score,
            baseline_updates_blocked=blocked,
            weights={
                "ml_anomaly": W_ML,
                "drift": W_DRIFT,
                "drift_confirmed_bonus": W_DRIFT_CONFIRMED,
                "policy_per_high_conf": W_POLICY_PER_HIGH,
                "policy_cap": W_POLICY_CAP,
            },
        )
