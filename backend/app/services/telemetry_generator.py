...
from app.config.device_profiles import (
    DEVICE_PROFILES,
...
class TelemetryGenerator:
    ...
    def generate_window(
        self,
        records_per_device: int = 3,
        attack_devices: Optional[dict[str, str]] = None,
    ...
    def get_telemetry_for_device(self, device_id: str) -> list[TelemetryRecord]:
        return [r for r in self.telemetry_store if r.device_id == device_id]

    def get_all_telemetry(self) -> list[TelemetryRecord]:
        return self.telemetry_store

    def get_latest_window(self) -> list[TelemetryRecord]:
        return [r for r in self.telemetry_store if r.window_id == self.window_counter]
