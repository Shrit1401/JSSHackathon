...
from app.config.device_profiles import DEVICE_PROFILES
...
class BaselineEngine:
    ...
    def learn_all_baselines(self, all_features: list[DeviceFeatureVector]) -> dict:
        ...
        for device_id, features in by_device.items():
            ...
            self.learn_device_baseline(device_id, device_type, features)

        for device_type, features in by_type.items():
            self.learn_type_baseline(device_type, features)

        all_windows = set(fv.window_id for fv in all_features)
        self.baseline_windows = len(all_windows)
    ...
    def compute_deviation(self, feature_vector: DeviceFeatureVector, threshold: float = 2.5) -> BaselineDeviation:
        ...
        if not baseline:
            ...
            type_bl = self.type_baselines.get(device_type)
            if not type_bl:
        ...
        return BaselineDeviation(
            ...
            max_deviation_feature=max_feat,
            max_deviation_zscore=round(abs_devs[max_feat], 4),
            features_beyond_threshold=beyond,
    ...
    def update_device_baseline(self, device_id: str, new_feature: DeviceFeatureVector) -> bool:
        ...
        for feat in NUMERIC_FEATURES:
            ...
            updated = FeatureStats(
                ...
                max_val=round(max(stats.max_val, new_val), 4),
                samples=n + 1,
            )
    ...
    def get_all_device_baselines(self) -> list[DeviceBaseline]:
        return list(self.device_baselines.values())

    def get_all_type_baselines(self) -> list[DeviceTypeBaseline]:
        return list(self.type_baselines.values())

    def get_summary(self) -> BaselineSummary:
        frozen = sum(1 for b in self.device_baselines.values() if b.is_frozen)
        return BaselineSummary(
            total_device_baselines=len(self.device_baselines),
            total_type_baselines=len(self.type_baselines),
            frozen_devices=frozen,
            baseline_phase_windows=self.baseline_windows,
            device_types_covered=[dt.value for dt in self.type_baselines],
        )

    def reset(self):
        self.device_baselines.clear()
        self.type_baselines.clear()
        self.baseline_windows = 0
